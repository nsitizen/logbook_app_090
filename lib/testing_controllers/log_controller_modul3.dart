...
class LogController {
  ...
  Future<void> loadFromDisk(String username) async {
    ...
    final String? jsonString = prefs.getString('user_logs_$username');

    if (jsonString != null) {
      final List decodedList = jsonDecode(jsonString);
      logsNotifier.value = decodedList.map((e) => LogModel.fromMap(e)).toList();
    } else {
      logsNotifier.value = [];
    }
  ...
  // CREATE & SAVE
  Future<void> addLog(String username, String title, String desc, String category) async {
  ...
  // UPDATE & SAVE
  Future<void> updateLog(String username, int index, String title, String desc, String category) async {
  ...
  // SIMPAN KE SHREDPREFERENCES
  Future<void> saveToDisk(String username) async {
    final prefs = await SharedPreferences.getInstance();
    final listMap = logsNotifier.value.map((log) => log.toMap()).toList();
